...
confidence fallback, CLI, Telegram-інтерфейс і локальну
...
→ grounded prompt
→ OpenAI gpt-4.1-mini або local Qwen + LoRA
...
provider недоступний, але OpenAI provider продовжує працювати.
...
При мережевій/API-помилці OpenAI виконується явний перехід на
local provider, якщо CUDA й adapter доступні. Помилка не
...
- `/provider openai|local` — provider поточного чату;
- `/status` — доступність OpenAI/CUDA/adapter і ліміти;
...
- OpenAI і реальний Telegram polling потребують приватних ключів
  та не були live-перевірені в цьому середовищі;
