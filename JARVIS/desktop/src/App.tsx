...
const UI = {
  en: {
    ...
    providerAuto: "Provider: Auto", evidenceOnly: "Evidence only", corpusProject: "Corpus: current project", corpusAll: "Corpus: all projects", file: "File", askFiles: "Ask about imported files", welcome: "JARVIS searches the selected corpus with FTS5, multilingual FAISS and BGE reranking, then shows exact source citations.",
    ...
    settingsTitle: "JARVIS RAG settings", settingsSubtitle: "Keys stay only in the local Windows application profile.", language: "Interface language", provider: "Provider", freeModel: "FreeModel model", freeModelKey: "FreeModel API key", openAiKey: "OpenAI API key", available: "available", notConfigured: "not configured", configured: "Configured — leave blank unchanged", saved: "Saved locally. Provider availability was refreshed.", desktopOnly: "JARVIS is desktop-only and uses these keys only for grounded answers over your selected local corpus.", close: "Close", save: "Save locally", saving: "Saving…", page: "page",
  },
  ru: {
    ...
    settingsTitle: "Настройки JARVIS RAG", settingsSubtitle: "Ключи сохраняются только в локальном профиле приложения Windows.", language: "Язык интерфейса", provider: "Провайдер", freeModel: "Модель FreeModel", freeModelKey: "API-ключ FreeModel", openAiKey: "API-ключ OpenAI", available: "доступен", notConfigured: "не настроен", configured: "Настроен — оставьте пустым, чтобы не менять", saved: "Сохранено локально. Доступность провайдеров обновлена.", desktopOnly: "JARVIS работает только на этом ПК и использует ключи лишь для grounded-ответов по выбранным локальным файлам.", close: "Закрыть", save: "Сохранить локально", saving: "Сохранение…", page: "стр.",
  },
} as const;
...
export function App() {
  ...
  return <main className="app" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); void handleFiles(event.dataTransfer.files); }}>
    ...
    {settingsOpen && <SettingsModal language={language} policy={policy} status={settingsStatus} draft={settingsDraft} notice={settingsNotice} busy={busy} onPolicy={changePolicy} onDraft={setSettingsDraft} onSave={saveSettings} onClose={() => setSettingsOpen(false)}/>}
  </main>;
}
...
function SettingsModal({ language, policy, status, draft, notice, busy, onPolicy, onDraft, onSave, onClose }: { language: UiLanguage; policy: Policy; status?: LocalSettingsStatus; draft: LocalSettingsDraft; notice: string; busy: boolean; onPolicy: (next: Partial<Policy>) => Promise<void>; onDraft: (next: LocalSettingsDraft) => void; onSave: () => Promise<void>; onClose: () => void }) {
  const update = (name: keyof LocalSettingsDraft, value: string) => onDraft({ ...draft, [name]: value });
  const text = UI[language];
  const secretFields: Array<[keyof LocalSettingsDraft, string]> = [["freemodel_api_key", text.freeModelKey], ["openai_api_key", text.openAiKey]];
  return <div className="modal-backdrop" onClick={onClose}><section className="modal settings-modal" onClick={(event) => event.stopPropagation()}><header><div><h2>{text.settingsTitle}</h2><small>{text.settingsSubtitle}</small></div><button onClick={onClose}><X/></button></header><div className="provider-status">{Object.entries(status?.providers || {}).map(([name, value]) => <span key={name} className={value.available ? "available" : "unavailable"}><i/>{name}: {value.available ? text.available : text.notConfigured}</span>)}</div><div className="settings-grid"><label>{text.provider}<select value={policy.provider_profile} onChange={(event) => void onPolicy({ provider_profile: event.target.value as Policy["provider_profile"] })}><option value="auto">{text.providerAuto}</option><option value="freemodel">FreeModel</option><option value="openai">OpenAI</option><option value="extractive">{text.evidenceOnly}</option></select></label><label>{text.freeModel}<input value={draft.freemodel_model ?? status?.models.freemodel ?? "auto"} onChange={(event) => update("freemodel_model", event.target.value)}/></label>{secretFields.map(([name, label]) => <label key={name}>{label}<input type="password" autoComplete="off" value={draft[name] || ""} placeholder={status?.configured[name] ? text.configured : text.notConfigured} onChange={(event) => update(name, event.target.value)}/></label>)}</div>{notice && <p className="settings-notice">{notice}</p>}<p>{text.desktopOnly}</p><footer><button className="ghost" onClick={onClose}>{text.close}</button><button className="primary" disabled={busy} onClick={() => void onSave()}>{busy ? text.saving : text.save}</button></footer></section></div>;
}
