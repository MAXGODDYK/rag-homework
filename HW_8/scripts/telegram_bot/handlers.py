...
from scripts.agent_flow import AgentState, MAX_GOAL_LENGTH, run_agent
...
async def start_command(
    ...
    text = (
        ...
        "Локальна команда /plan запускає deterministic agent workflow "
        "без LLM та зовнішніх API.\n\n"
        "Команди: /help, /provider, /rate, /plan, /trace, "
...
async def help_command(
    ...
    text = (
        ...
        "/plan Як підготуватися до іспиту? — локальний agent flow\n"
        "/trace — steps і state останнього agent flow\n"
...
async def status_command(
    ...
    lines = [
        ...
        "Agent flow: available — local deterministic, API не потрібен",
    ]
    await update.effective_message.reply_text("\n".join(lines))
...
async def sources_command(
    ...
    agent_state: AgentState | None = context.user_data.get(
        "last_agent_state"
    )
    if agent_state is not None:
        await update.effective_message.reply_text(
            "Остання відповідь створена локальними mock tools без "
            "зовнішніх джерел. Використайте /trace для перегляду steps."
        )
        return
...
async def reset_command(
    ...
    context.user_data.pop("last_agent_state", None)
    context.user_data["provider"] = DEFAULT_PROVIDER
    await update.effective_message.reply_text(
        "Provider скинуто до openai; останній RAG/tool/agent "
        "результат очищено."
    )
...
def format_agent_answer(state: AgentState) -> str:
    tool_names = ", ".join(
        call.tool_name for call in state.tool_calls
    ) or "none"
    return (
        f"Agent route: {state.selected_route}\n"
        f"Tools: {tool_names}\n\n"
...
async def plan_command(
    ...
    if not question:
        await message.reply_text(
            "Використання: /plan <навчальна мета>.\n"
            "Приклад: /plan Як підготуватися до іспиту?"
        )
        return
    if len(question) > MAX_GOAL_LENGTH:
        await message.reply_text(
            f"Запит занадто довгий. Максимум: {MAX_GOAL_LENGTH} символів."
        )
        return
    ...
    try:
        state = await asyncio.to_thread(run_agent, question)
    except ValueError as error:
        await message.reply_text(f"Agent validation: {error}")
        return

    context.user_data.pop("last_result", None)
    context.user_data.pop("last_tool_result", None)
    context.user_data["last_agent_state"] = state
    await _send_long_message(update, format_agent_answer(state))
...
async def trace_command(
    ...
    state: AgentState | None = context.user_data.get("last_agent_state")
    if state is None:
        await update.effective_message.reply_text(
            "Ще немає agent trace. Спочатку виконайте /plan <запит>."
        )
        return
